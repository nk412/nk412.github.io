...
(single input only). Pass --short-edge to change the target size and
--quality to change the AVIF quality.
"""
...
def convert(src: Path, dst: Path, short_edge: int = SHORT_EDGE, quality: int = QUALITY) -> None:
...
def main() -> None:
    ...
    p.add_argument("--quality", type=int, default=QUALITY, help=f"AVIF quality 0-100 (default {QUALITY})")
    args = p.parse_args()
    ...
    for src in args.images:
        ...
        convert(src, out_dir / f"{stem}.avif", args.short_edge, args.quality)
